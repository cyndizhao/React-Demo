...
export function Shape(props) {
  ...
  if (left !== undefined || bottom !== undefined) {
    Object.assign(style, { left, bottom, position: 'absolute' })
  }

  return <div onClick={onClick} className='Shape' style={style}></div>
}
...
export default class ShapeMover extends Component {
  ...
  moveShape(event) {
    ...
    switch (target.name) {
      case 'left':
        return this.setState({x: x - 5})
      case 'up':
        return this.setState({y: y + 5})
      case 'down':
        return this.setState({y: y - 5})
      case 'right':
        return this.setState({x: x + 5})
...
//
// class ShapeMover extends Component {
//   constructor (props) {
//     super(props);
//
//     this.state = {
